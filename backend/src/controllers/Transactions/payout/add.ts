...
const createPayout = expressAsyncHandler(
    ...
    try {
      ...
      if (model === 'supplier') {
        ...
        if (req.role === 'admin') {
          ...
          const isBank = await Bank.findById(toID).session(session);
        ...
        if (isExist) {
          ...
          if (req.role === 'admin') {
            ...
            const isBank = await Bank.findById(fromID).session(session);
            if (!isBank) {
      ...
      if (req.role !== 'admin') {
        const isBank = await Bank.findOne({
          ...
          .sort({ createdAt: -1 });
        if (!isBank) {
          throwError(`your store has no account for payouts`);
          return;
        }
